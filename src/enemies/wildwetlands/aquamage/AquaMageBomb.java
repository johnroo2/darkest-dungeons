...
public class AquaMageBomb extends Projectile{
	final static double ARRIVAL_FRAMES = 110;
	final static Color BOMB_COLOUR = new Color(70, 70, 240);
	
	int initX, initY;
	int destX, destY;
	double angle;
	
	public AquaMageBomb(int x, int y, int range, int ori) {
	...
	@Override
	public void destroy() {
		new AquaMageSlam((int)this.x, (int)this.y);
	}
...
class AquaMageSlam extends AoE{
	int dmg;
	boolean hit = false;
	
	public AquaMageSlam(int x, int y) {
		super(x, y, AquaMage.RADIUS_AOE, AOE_DARKBLUE, 25);
		this.dmg = AquaMage.DMG_AOE;
	...
	public boolean ranged() {
		...
		if(!hit && this.inRadius(Controller.chr.getX(), Controller.chr.getY(), (int)(AquaMage.RADIUS_AOE * 1.1))) {
			hit = true;
			Controller.chr.damage("Aqua Mage Bomb", this.dmg);
			new AllyQuiet(83); //0.5 seconds
		}
		return false;
	}
}
