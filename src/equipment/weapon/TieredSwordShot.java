...
public class TieredSwordShot extends Projectile{
	final int SIZE = 40;
	
	int type, size, minDmg, maxDmg;
	BufferedImage img;
	
	public TieredSwordShot(int x, int y, int targetX, int targetY, double speed, int range, int type, int minDmg, int maxDmg) {
		...
		this.size = (int) (SIZE * Rotate.getSizeConstant(rotation) * 1.35);
		
		this.img = Rotate.rotateImage(Projectile.BULLETS_SWORD.get(type), rotation);
